...
st.set_page_config(page_title="Medicine Recommendation App",layout="wide",page_icon = im)
...
with col2:
    st.markdown("<div style='display: flex; background-color: #3498DB; color: #2C3E50; justify-content: center; align-items: center; height: 100%;'><h1>Health Care Center</h1></div>", unsafe_allow_html=True)

st.markdown("""
Please enter your symptoms separated by commas (e.g., itching, skin_rash, nodal_skin_eruptions).
""")
...
if st.button('Predict', key='predict_button'):
    if not symptoms:
        ...
        st.error('Symptoms should only contain alphabetic characters.')
    else:
        ...
        st.markdown(f"""
        <div style=" padding: 10px; border-radius: 5px; color: #2C3E50;">
        ...
        for pre in precautions[0]:
            st.markdown(f"<li>{pre}</li>", unsafe_allow_html=True)
        ...
        for diet in rec_diet:
            st.markdown(f"<li>{diet}</li>", unsafe_allow_html=True)
        st.markdown("</ul></div>", unsafe_allow_html=True)
        ...
        """, unsafe_allow_html=True)
        for wrk in workout:
            st.markdown(f"<li>{wrk}</li>", unsafe_allow_html=True)
        st.markdown("</ul></div>", unsafe_allow_html=True)